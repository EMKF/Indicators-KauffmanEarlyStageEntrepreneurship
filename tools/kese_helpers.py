...
def _rne(df):
    """Calculate the Rate of New Entrepreneurs."""
    return (df['ent015ua'] * df['wgtat1']).sum() / df['wgtat1'].sum()


def _ose(df):
    """Calculate the Opportunity Share of Entrepreneurs."""
    return (df['oppshare'] * df['wgtat1']).sum() / df['wgtat1'].sum()


def preprocess_cps(df, geo_level):
    """
    Pre-processes CPS data. Generate indicators and aggregate it to the annual
    level, broken down by category.
    ...
    print('\tPre-processing data for', geo_level, df['yeart1'].unique())
    ...
    if geo_level == 'state':
        ...
        df_processed = df_rne \
            ...
            .assign(
                ...
                region=lambda x: x['fips'].map(c.state_fips_abb_dic) \
                    .map(c.abbrev_us_state)
            ) \
            .drop(columns='state')
        ...
        for type_c in c.kese_categories:
            for cat in c.kese_categories[type_c]:
                ...
                df_processed = df_processed \
                    .append(df_rne \
                        .merge(df_ose) \
                        .assign(type=type_c, category=cat, fips='00')
                    ) \
...
def pep_pre_2000(geo_level):
    """Fetch population data for years: 1996 - 1999."""
    return pd.read_excel(
            'http://www2.census.gov/library/publications/2011/compendia/statab/131ed/tables/12s0013.xls?', 
            skiprows=3, 
            skipfooter=9
        ) \
        ...
        .query(
            'region == "  United States "' if geo_level == 'us' 
        ...
        .pipe(
            pd.melt, 
            id_vars='region', 
            value_vars=range(1996, 2000), 
            var_name='time', 
            value_name='population'
        ) \
        .assign(
            region=lambda x: x.region.str.strip(),
            fips=lambda x: x.region \
                .map(c.us_state_abbrev).map(c.state_abb_fips_dic),
...
def pep_2020(geo_level):
    ...
    if geo_level == 'us':
        return pd.read_excel(
                ...
                skiprows=2, 
                usecols=['Year and Month', 'Resident Population']
            ) \
            .iloc[128:139, :] \
            .rename(columns={
                'Year and Month':'time', 'Resident Population': 'population'
            }) \
            .query('time == ".July 1"') \
            .assign(
                time=2020,
